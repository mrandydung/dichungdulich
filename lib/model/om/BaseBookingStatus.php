...
abstract class BaseBookingStatus extends BaseObject  implements Persistent {
	...
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = BookingStatusPeer::ID;
		}
	...
	public function setName($v)
	{
		...
		if ($this->name !== $v) {
			$this->name = $v;
			$this->modifiedColumns[] = BookingStatusPeer::NAME;
		}
	...
	public function hydrate(ResultSet $rs, $startcol = 1)
	{
		try {
			...
			throw new PropelException("Error populating BookingStatus object", $e);
		}
	}
	...
	public function delete($con = null)
	{
		...
		try {
			$con->begin();
			BookingStatusPeer::doDelete($this, $con);
	...
	protected function doSave($con)
	{
		$affectedRows = 0; 		if (!$this->alreadyInSave) {
			$this->alreadyInSave = true;
						...
						if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = BookingStatusPeer::doInsert($this, $con);
					$affectedRows += 1; 										 										 
					$this->setId($pk);  
					$this->setNew(false);
				} else {
					$affectedRows += BookingStatusPeer::doUpdate($this, $con);
				}
				$this->resetModified(); 			}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = BookingStatusPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = BookingStatusPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = BookingStatusPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getName(),
		);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(BookingStatusPeer::ID)) $criteria->add(BookingStatusPeer::ID, $this->id);
		if ($this->isColumnModified(BookingStatusPeer::NAME)) $criteria->add(BookingStatusPeer::NAME, $this->name);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(BookingStatusPeer::ID, $this->id);
	...
	{
		if (self::$peer === null) {
			self::$peer = new BookingStatusPeer();
		}
		return self::$peer;
	}
		...
		if ($this->collBookingTours === null) {
			if ($this->isNew()) {
			   $this->collBookingTours = array();
				...
				$criteria->add(BookingTourPeer::BOOKING_STATUS_ID, $this->getId());
