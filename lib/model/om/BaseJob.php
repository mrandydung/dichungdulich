...
abstract class BaseJob extends BaseObject  implements Persistent {
	...
	public function setPriority($v)
	{
		...
		if ($this->priority !== $v) {
			$this->priority = $v;
			$this->modifiedColumns[] = JobPeer::PRIORITY;
		}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(JobPeer::PRIORITY)) $criteria->add(JobPeer::PRIORITY, $this->priority);
		...
		if ($this->collDichungUsers === null) {
			if ($this->isNew()) {
			   $this->collDichungUsers = array();
				...
				$criteria->add(DichungUserPeer::JOB_ID, $this->getId());
