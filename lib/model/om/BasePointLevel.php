...
abstract class BasePointLevel extends BaseObject  implements Persistent {
	...
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = PointLevelPeer::ID;
		}
	...
	public function setName($v)
	{
		...
		if ($this->name !== $v) {
			$this->name = $v;
			$this->modifiedColumns[] = PointLevelPeer::NAME;
		}
	...
	public function setPoint($v)
	{
		...
		if ($this->point !== $v) {
			$this->point = $v;
			$this->modifiedColumns[] = PointLevelPeer::POINT;
		}
	...
	public function hydrate(ResultSet $rs, $startcol = 1)
	{
		try {
			...
			$this->point = $rs->getInt($startcol + 2);
			...
			throw new PropelException("Error populating PointLevel object", $e);
		}
	}
	...
	public function delete($con = null)
	{
		...
		try {
			$con->begin();
			PointLevelPeer::doDelete($this, $con);
	...
	protected function doSave($con)
	{
		$affectedRows = 0; 		if (!$this->alreadyInSave) {
			$this->alreadyInSave = true;
						...
						if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = PointLevelPeer::doInsert($this, $con);
					$affectedRows += 1; 										 										 
					$this->setId($pk);  
					$this->setNew(false);
				} else {
					$affectedRows += PointLevelPeer::doUpdate($this, $con);
				}
				$this->resetModified(); 			}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = PointLevelPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = PointLevelPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = PointLevelPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getName(),
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(PointLevelPeer::ID)) $criteria->add(PointLevelPeer::ID, $this->id);
		if ($this->isColumnModified(PointLevelPeer::NAME)) $criteria->add(PointLevelPeer::NAME, $this->name);
		if ($this->isColumnModified(PointLevelPeer::POINT)) $criteria->add(PointLevelPeer::POINT, $this->point);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(PointLevelPeer::ID, $this->id);
		...
		if ($this->collDichungUsers === null) {
			if ($this->isNew()) {
			   $this->collDichungUsers = array();
				...
				$criteria->add(DichungUserPeer::POINT_LEVEL_ID, $this->getId());
