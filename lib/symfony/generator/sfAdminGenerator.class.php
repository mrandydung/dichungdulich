 ...
 * @version    SVN: $Id: sfAdminGenerator.class.php 9862 2008-06-25 12:07:13Z fabien $
 */
abstract class sfAdminGenerator extends sfCrudGenerator
{
  ...
  public function getColumnEditTag($column, $params = array())
  {
    ...
    elseif(strtoupper($column->getName()) == 'JSON') {
      $ret = "\$json = \${$this->getSingularName()}->getJson();\n";
      $ret .="
      \$row = '<div class=\"json_row_container\">';
      if(\$json) { 
         
        foreach(\$json as \$k => \$v) {
          \$row .= '<p class=\"json_row\">';
          \$row .= '<input class=\"json_key\" name=\"{$this->getSingularName()}[".$column->getName()."][key][]\" value=\"'.\$k.'\"/>';
          \$row .= '<textarea class=\"json_value\" name=\"{$this->getSingularName()}[".$column->getName()."][value][]\">'.\$v.'</textarea>';
          \$row .= '<button class=\"remove\">REMOVE</button>';
          \$row .= '</p>';
      ...
      \$row .='<p class=\"json_action\"><button class=\"add\">+ADD</button></p>';
      \$row .= '
      <script>
       $(document).ready(function() {
         
        function removeJson() {
          $(\'#sf_admin_content .json_row .remove\').click(function() {
            $(this).parent().remove();
            return false;
          });
        }
        ...
        $(\'#sf_admin_content .json_action .add\').click(function() {
          $(\'.json_row_container\').append(\'<p class=\"json_row\"><input class=\"json_key\" name=\"{$this->getSingularName()}[".$column->getName()."][key][]\" /><textarea class=\"json_value\" name=\"{$this->getSingularName()}[".$column->getName()."][value][]\"></textarea><button class=\"remove\">REMOVE</button></p>\');
          removeJson();
          return false;
        });
      });
