 ...
 * @author     hungn
 * @version    SVN: $Id: actions.class.php 31/12/2014
 */
...
<?php endif; ?>
  ...
  public function executeEdit()
  {
    ...
    if ($this->getRequest()->isMethod('post'))
    {
      ...
      $this->getUser()->setFlash('notice', 'Lưu thành công');
...
<?php $upload_dir = $this->replaceConstants($this->getParameterValue('edit.fields.'.$column->getName().'.upload_dir')) ?>
      //$currentFile = sfConfig::get('sf_upload_dir')."/<?php echo $upload_dir ?>/".$this-><?php echo $this->getSingularName() ?>->get<?php echo $column->getPhpName() ?>();
			$currentFile = sfConfig::get('sf_upload_dir').$this-><?php echo $this->getSingularName() ?>->get<?php echo $column->getPhpName() ?>();
...
<?php endif; ?>
      // Update many-to-many for "<?php echo $name ?>"
			...
			$oId = $<?php echo $this->getSingularName() ?>->getPrimaryKey();
			$objIds = array();
      ...
      if (is_array($ids))
      {
        ...
        {
					$c = new Criteria;
					$c->add(<?php echo $through_class ?>Peer::<?php echo strtoupper($column->getColumnName()) ?>, $oId);
					$c->add(<?php echo $through_class ?>Peer::<?php echo strtoupper($related_column->getColumnName()) ?>, $id);
					$<?php echo ucfirst($through_class) ?> = <?php echo ucfirst($through_class) ?>Peer::doSelectOne($c);
					if(!$<?php echo ucfirst($through_class) ?> ){
						$<?php echo ucfirst($through_class) ?> = new <?php echo $through_class ?>();
						$<?php echo ucfirst($through_class) ?>->set<?php echo $column->getPhpName() ?>($oId);
						$<?php echo ucfirst($through_class) ?>->set<?php echo $related_column->getPhpName() ?>($id);
						$<?php echo ucfirst($through_class) ?>->save();
					}
					$objIds [] = $<?php echo ucfirst($through_class) ?>->getId();
        }
      }
			
			$c = new Criteria();
      $c->add(<?php echo $through_class ?>Peer::<?php echo strtoupper($column->getColumnName()) ?>, $oId);
			$c->add(<?php echo $through_class ?>Peer::ID, $objIds, Criteria::NOT_IN);
...
<?php $upload_dir = $this->replaceConstants($this->getParameterValue('edit.fields.'.$column->getName().'.upload_dir')) ?>
    //$currentFile = sfConfig::get('sf_upload_dir')."/<?php echo $upload_dir ?>/".$this-><?php echo $this->getSingularName() ?>->get<?php echo $column->getPhpName() ?>();
		$currentFile = sfConfig::get('sf_web_dir').$this-><?php echo $this->getSingularName() ?>->get<?php echo $column->getPhpName() ?>();
...
<?php endif ?>
			...
			$ext = pathinfo($_FILES['<?php echo $this->getSingularName() ?>']['name']['<?php echo $name ?>']);
      ...
      $this-><?php echo $this->getSingularName() ?>->set<?php echo $column->getPhpName() ?>("/uploads/<?php echo $upload_dir ?>/".$fileName.$ext);
...
<?php elseif (strtoupper($column->getPhpName()) == 'JSON'): ?>
    $jsonArr = $<?php echo $this->getSingularName() ?>['<?php echo $name ?>'];
    $json = array();
    for($i = 0; $i < sizeof($jsonArr['key']); $i++) {
      $k = $jsonArr['key'][$i];
      $v = $jsonArr['value'][$i];
      $json [$k] = $v;
    }
    $this-><?php echo $this->getSingularName() ?>->set<?php echo $column->getPhpName() ?>(json_encode($json));
...
<?php else: ?>
    else if (isset($this->filters['<?php echo $column->getName() ?>']) && $this->filters['<?php echo $column->getName() ?>'] !== '' && defined('<?php echo $this->getPeerClassName() ?>::<?php echo strtoupper($column->getName()) ?>'))
    {
